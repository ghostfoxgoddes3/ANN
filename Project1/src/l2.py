...
def train_model(split_name):
    ...
    torch.save(
        model.state_dict(),
        f"results/l2_{split_name}.pt",
    )

    history_df.to_csv(
        f"results/l2_{split_name}_history.csv",
        index=False,
    )
    ...
    plt.ylabel("MSE")
    plt.title(
        f"L2 - {split_name.capitalize()} split"
    )
    plt.legend()
    plt.grid(True)

    plt.savefig(
        f"figures/l2_{split_name}_curves.png",
        dpi=300,
        bbox_inches="tight",
    )
