...
def train_momentum(split_name, config):
    set_seed(SEED)
    ...
    history_df.to_csv(
        f"results/momentum_{split_name}_history.csv",
        index=False,
    )

    torch.save(
        model.state_dict(),
        f"results/momentum_{split_name}.pt",
    )
...
def plot_history(history_df, split_name):
    ...
    plt.savefig(
        f"figures/momentum_{split_name}_curves.png",
        dpi=300,
        bbox_inches="tight",
    )
...
def main():
    ...
    for split_name, config in CONFIGS.items():
        ...
        history, best_epoch, best_val_mse = train_momentum(
            split_name,
            config,
        )
        ...
        print(f"validation MAE: {best_row['val_mae']:.6f}")
        print(f"epochs: {len(history)}")

    results_df = pd.DataFrame(results)

    results_df.to_csv(
        "results/momentum_results.csv",
        index=False,
    )
